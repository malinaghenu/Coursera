...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df, values='class',labels= {'class':'Total'},names='Launch Site',title='Total Successes for All Launch Sites')
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]["class"].value_counts().to_frame()
        fig = px.pie(filtered_df, values='class', names=['success','failure'], title='Total Success Launches for ' + entered_site)
    return fig
# TASK 4:
